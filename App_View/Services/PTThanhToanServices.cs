...
namespace App_View.Services
{
    public class PTThanhToanServices : IPTThanhToanServices
    {
        ...
        public async Task<bool> CreatePTThanhToanAsync(string ten, string mota, int trangthai)
        {
            try
            {
                var res = await _httpClient.PostAsync($"https://bazaizaiapi-v2.azurewebsites.net/api/PTThanhToan?ten={ten}&mota={mota}&trangthai={trangthai}", null);
        ...
        {
            return await _httpClient.GetFromJsonAsync<List<PhuongThucThanhToan>>("https://bazaizaiapi-v2.azurewebsites.net/api/PTThanhToan");
        }
