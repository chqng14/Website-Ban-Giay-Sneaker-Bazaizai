    ...
    public class VoucherNguoiDungServices : IVoucherNguoiDungServices
    {
        ...
        public async Task<bool> AddVoucherNguoiDung(string MaVoucher, string idNguoiDung)
        {
            try
            {
                ...
                string apiUrl = $"https://bazaizaiapi-v2.azurewebsites.net/api/VoucherNguoiDung/AddVoucherNguoiDung?MaVoucher={MaVoucher}&idNguoiDung={idNguoiDung}";
        ...
        public async Task<bool> TangVoucherNguoiDungMoi(string ma)
        {
            ...
            {
                var httpClient = new HttpClient();
                string apiUrl = $"https://bazaizaiapi-v2.azurewebsites.net/api/VoucherNguoiDung/TangVoucherChoNguoiDungMoi?ma={ma}";
                var response = await httpClient.PostAsync(apiUrl, null);
                var check = await response.Content.ReadAsStringAsync();
                if (check == "true")
                {
